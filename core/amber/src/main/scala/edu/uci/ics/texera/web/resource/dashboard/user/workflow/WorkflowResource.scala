...
object WorkflowResource {
  ...
  private def insertWorkflow2(workflow: Workflow): Unit = {
    workflowDao.insert(workflow)
    val uIntegerValue1 = UInteger.valueOf(1)
    workflowOfUserDao.insert(new WorkflowOfUser(uIntegerValue1, workflow.getWid))
    workflowUserAccessDao.insert(
      new WorkflowUserAccess(
        uIntegerValue1,
        workflow.getWid,
...
class WorkflowResource {
  ...
  @POST
  @Path("/upload")
  def uploadWorkflow(json: String): Response = {
    val workflowData: Map[String, Any] = Map(
      "name" -> "Workflow",
      "content" -> json
    )
    val objectMapper = new ObjectMapper()
    objectMapper.registerModule(DefaultScalaModule)
    val workflowJson = objectMapper.writeValueAsString(workflowData)
    val workflow = objectMapper.readValue(workflowJson, classOf[Workflow])
    if (workflow.getWid != null) {
      throw new BadRequestException("Cannot create a new workflow with a provided id.")
    } else {
      insertWorkflow2(workflow)
      WorkflowVersionResource.insertVersion(workflow, insertNewFlag = true)
      val dashboardWorkflow = DashboardWorkflow2(workflow.getWid)
      Response
         .ok(dashboardWorkflow)
